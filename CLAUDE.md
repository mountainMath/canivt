...
| `read.R`        | public `read_ivt()`, `ivt_metadata()`, `ivt_tidy()`, `print.ivt` — **one path for all families** (decode via `ivt_decode()`, metadata via `ivt_f2_metadata()`; `family` now only tags provenance and gates the `geo_attributes` option); `ivt_family()` detector + `ivt_is_supported()` gate. `ivt_tidy(dim_names=)` names data columns by the **full English dimension label** (`"label"`, default — e.g. `Age of primary household maintainer`) or the terse structural **slug** (`"slug"` — e.g. `age`); `x$cells` always keeps slugs (the decoder stays name-agnostic), so the label naming is an output-layer choice (`ivt_data_colnames()`, read-f2.R) shared by `ivt_tidy()`/`ivt_members()` so parquet + sidecar agree. Threads through `ivt_write_parquet()`/`ivt_write_csv()`/`get_statcan_ivt()`/`collect_ivt()`. Geography columns (`geo_name`/`geo_uid`/…) are unaffected. |
