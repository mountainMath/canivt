...
| `read.R`        | public `read_ivt()`, `ivt_metadata()`, `ivt_tidy()`, `print.ivt` — **one path for all families** (decode via `ivt_decode()`, metadata via `ivt_f2_metadata()`; `family` now only tags provenance and gates the `geo_attributes` option); `ivt_family()` detector + `ivt_is_supported()` gate. `ivt_tidy(dim_names=)` names data columns by the **full dimension label** (`"label"`, default — e.g. `Age of primary household maintainer`) or the terse structural **slug** (`"slug"` — e.g. `age`); `x$cells` always keeps slugs (the decoder stays name-agnostic), so the label naming is an output-layer choice (`ivt_data_colnames()`, read-f2.R) shared by `ivt_tidy()`/`ivt_members()` so parquet + sidecar agree. `ivt_tidy(language=)` outputs **English (`"en"`, default) or French (`"fr"`)** labels + label-derived column names (`ivt_norm_lang()` normalises `en`/`eng`/`fr`/`fra`/… lower-case); French falls back to English per-column where the file carries no French copy (geo_uid, Statistics). Both thread through `ivt_write_parquet()`/`ivt_write_csv()`/`get_statcan_ivt()`/`collect_ivt()`; the `_members.parquet` sidecar carries `level`+`level_fr` so `collect_ivt(language="fr")` factors on `level_fr`. Geography columns keep their `geo_*` names (French *content* in fr mode); `geo_uid` is language-neutral. |
...
- Member id columns in `cells` are **1-based**. `cells` data columns are named by a
  ...
  **`ivt_tidy()`/parquet output** defaults to the **full English dimension label**
  as the data column name (`dim_names = "label"`; `"slug"` restores the terse
  names) — but that is an output-layer rename in `ivt_data_colnames()`; the
  decoder and `x$cells` stay on slugs, so nothing structural depends on it.
