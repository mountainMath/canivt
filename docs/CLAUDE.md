...
- **The bitmap addresses members by SLOT, and slots can have holes.**
  ...
  (`ivt_f2_time_members()`, markers.md §E.1): `alloc` is a full **u16**
  slot capacity (long monthly series exceed 255 — LFHR `Table-023`’s
  ...
  right-aligned, **days since 0000-03-01** (lands on Jan 1 for annual
  series, the ISO month-start for monthly; labels are GENERATED from the
  dates). When slots ≠ 1..count, the descriptor dim carries `$slots` and
  the layout is slot-aware: extents drive nesting/window/stride
  geometry, `ivt_f2_cell_grid(pos=)` maps member bits to slot positions,
